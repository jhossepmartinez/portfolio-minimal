...
The project shipped three semantic versions over 10 days, with each version representing a deliberate scope boundary rather than arbitrary milestones.

**v1.0.0** established the core: a working AI endpoint, automated releases, and semantic versioning from the first commit. **v2.0.0** introduced persistent state — a breaking change, acknowledged as such, with the Supabase-to-D1 tradeoff resolved during this phase. **v2.1.0** completed the security and operational layer: environment isolation, OAuth, rate limiting, and observability all landed together.

Post-release work focused on correctness and maintainability — DB normalization for rate-limit integrity, history scoping to authenticated users, and a structural refactor for cleaner separation of concerns.
